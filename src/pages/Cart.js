...
import { decrementQuantity, deleteItem,resetCart, increamentQuantity } from '../redux/amazonSlic';
...
const Cart = () =>{
  const dispatch =useDispatch()
    ...
    useEffect(() => {
      ...
      return setTotalprice(Total.toFixed())})
    ...
    <div  className='w-full bg-gray-100 p-4'>
       {
        ...
        <div className='w-full h-full bg-white px-4 col-span-4'>
          ...
          <div>{
            ...
            <div key={item.id} className='w-full border-b-[1px] border-b-gray-300 p-4 flex items-center gap-6'>
              ...
              <div className='w-1/5'>
                   <img className='h-44 object-contain' src={item.image} alt="Productimage"/>
                </div>
                <div className='w-3/5'>
                  <h2 className='font-semibold text-lg'>{item.title}</h2>
                  <p className='pr-10 text-sm'>{item.description.substring(0,200)}...</p>
                  <p className='text-base'>Unit Price: <span className='font-sembold'>${item.price}</span></p>
                  <div className='bg-[#F0F2F2] flex justify-center items-center gap-1 w-24 py-1 text-center drop-shadow-lg rounded-md'>
                    ...
                    <p>{item.quantity}</p>
                    <p onClick={()=>dispatch(increamentQuantity(item.id))} 
                    className='cursor-pointer bg-gray-200 px-1 rounded-md hover:bg-gray-400 duration-300'>+</p>
                  </div>
        ...
        <div className='w-full h-52 bg-white col-span-1 flex flex-col justify-center items-center p-4 '>
          ...
          <div>
            <p className='font-semibold px-10 py-1 flex items-center gap-3 justify-between'>
       ...
       <img
        ...
        src={emptyCart} alt ="emptycartiamge"/>
       </div>
