...
const Products = ({image}) => {
  ...
  return (
    <div className='max-w-screen-2xl mx-auto grid grid-cols-4 gap-10  px-4'>
      {
        productData.map((item)=>(<div key={item.id} className='bg-white h-auto border-[1px] border-gray-200 py-6 z-30 hover:border-transparent shadow-none hover:shadow-testShadow duration-200 flex flex-col gap-4 relative'>
            <span className='text-xs capitalize italic absolute top-2 right-0 text-gray-500'> {item.category}</span>
          <div className='w-full h-auto flex items-center justify-center relative'><img className='w-52 h-64 object-contain' src={item.image} alt ='product'/>
          <ul className='w-full h-36 bg-gray-100 absolute bottom-[-170px] flex flex-col items-end justify-center gap-2 font-titleFont px-2 border-1 border-r group-hover:bottom-0 duration-700'>
            ...
            <li className='productLi'>Add to Cart <span><ShoppingCartCheckoutIcon /></span></li>
        ...
        <div className='px-4 z-10'>
          ...
          <div>
            <p className='text-sm mb-1'>{item.description.substring(1,100)}...</p>
